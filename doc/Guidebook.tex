...
%
\item[\bb{Convicts}]%
are hardened by the prison system and ready for anything 
after their recent escape.  They may have the hard knock life in the 
early going, but convicts are just as tough as the dungeon.
%.pg
%
\item[\bb{Flame Mages}]%
...
%
\item[\bb{Jedi}]%
are masters of the force, the energy that flows through the universe. Their 
skills are focused on the sword and lightsaber; care for it well, and it will
keep you alive.
%.pg
%
\item[\bb{Knights}]%
...
%
\item[\bb{Pirates}]%
are swashbuckling adventurers, skilled on the sea and tough on land.
They speak the language of the sea and drink like fish. 
%.pg
%
...
have the enviable ability to change form at will,  at
...
%.lp
\item[\tb{\#tech}]
Perform a role or race specific technique. Autocompletes. Default key is `{\tt M-t}'.
%.lp
\item[\tb{\#twoweapon}]
...
%.lp
\item[\tb{\#wizspell}]
Cast any spell by name without cost or failure as an expert.
Autocompletes.
Debug mode only.
Default key is `{\tt \^{}S}'.
%.lp
\item[\tb{\#wiztelekinesis}]
Hurtle a monster across the map.
Autocompletes.
Debug mode only.
%.lp
\item[\tb{\#wizwhere}]
...
%.lp ""
On Unix, Linux and Mac OS X it is \mbox{``.hackemrc''} in the user's home
directory. The file may not exist, but it is a normal ASCII text file and
can be created with any text editor.\\

%.lp ""
On Windows, it is \mbox{``.hackemrc''} in the folder
...
template for ths configuration file named \mbox{``.hackemrc.template''} in
...
defaults to the location of the {\it HackEM.exe\/} or {\it NetHackw.exe\/} file
...
%.lp
\item[\ib{autostairtravel}]
Automatically travel to stairs using < or >.
%.lp
\item[\ib{autoquiver}]
...
%.lp
\item[\ib{ctrlkick}]
Enable kicking shortcuts with ctrl + direction.
%.lp
%.lp
...
%.lp
\item[\ib{birdname}]
Name your starting bird (for example, ``{\tt birdname:Squawks}'').
Cannot be set with the `{\tt O}' command.
%.lp
\item[\ib{dogname}]
...
%.lp
\item[\ib{monkeyname}]
Name your starting monkey (for example, ``{\tt monkeyname:Ugga-Ugga}'').
Cannot be set with the `{\tt O}' command.
%.lp
...
%.lp
\item[\ib{realtime}]
Display various types of time to show on the status bar.
Default is wallclock time.  Persistent.
%.lp
\item[\ib{realtime\verb+_+format}]
The format used by the realtime display.
Default is fixed units (hh:mm:ss).  Persistent.
%.lp
...
\verb@z@ & S\verb+_+zouthern                &	(zouthern)\\
