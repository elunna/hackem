...
boolean has_of;
{
    ...
    if (props & ITEM_RAGE) {
        if ((props_known & ITEM_RAGE) || dump_prop_flag) {
...
struct obj *no_wish;
{
    ...
    very = rechrg = blessed = uncursed = iscursed = magical = ispoisoned 
        = isgreased = eroded = eroded2 = erodeproof = halfeaten = islit 
        = unlabeled = ishistoric = isdiluted = trapped = locked = unlocked 
        = broken = 0;
    tvariety = RANDOM_TIN;
    mntmp = NON_PM;
#define UNDEFINED 0
...
#define SPINACH 2
                ...
                if ((mntmp = name_to_mon(p + of)) >= LOW_PM) {
                    *p = 0;
                    ...
                    if (!objpropcount || wizard)
                        objprops |= ITEM_TOUGH;
                    objpropcount++;
                } else if (!strncmpi((p + of), "prowess", l = strlen("prowess"))) {
                    if (!objpropcount || wizard)
                        objprops |= ITEM_PROWESS;
                    objpropcount++;
 ...
 typfnd:
    ...
    if (otmp->oclass == WEAPON_CLASS || is_weptool(otmp) 
        ...
        else if (objprops & ITEM_PROWESS)
            objprops &= ~(ITEM_RES_PROPS & ~ITEM_PROWESS);
        ...
        if (is_launcher(otmp))
            objprops &= ~((ITEM_RES_PROPS & ~NON_WEP_PROPS) 
                    | ITEM_OILSKIN | ITEM_RAGE);
