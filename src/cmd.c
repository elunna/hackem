...
domonability(VOID_ARGS)
{
    if (can_breathe(youmonst.data))
        ...
        return dospit();
    else if (attacktype(youmonst.data, AT_VOLY))
        return dovolley();
                                            ...
                                            : (context.warntype.polyd & MH_DEMON)
                                                  ...
                                                  : (context.warntype.polyd & MH_ANGEL)
                                                        ...
                                                        : (context.warntype.polyd & MH_JABBERWOCK)
                                                              ? "jabberwocks"
                                                              : (context.warntype.polyd & MH_VAMPIRE)
                                                                    ? "vampires"
                                                                    : "certain monsters");
        you_are(buf, "");
    }
...
boolean doit;
{
    ...
    if (IS_FOUNTAIN(typ) || IS_SINK(typ) || IS_TOILET(typ)) {
...
find_remembered_stairs(boolean upstairs, coord *cc)
{
    ...
    if (isok(x, y) &&
        (glyph_to_cmap(level.locations[x][y].glyph) == stair ||
         glyph_to_cmap(level.locations[x][y].glyph) == sladder ||
         glyph_to_cmap(level.locations[x][y].glyph) == sbranch)) {
        cc->x = x;
        cc->y = y;
        return TRUE;
    }
