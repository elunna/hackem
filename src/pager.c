...
#define MONPUTSTR(str) putstr(datawin, ATR_NONE, str)
    ...
    ADDRESIST(pm_resistance(pm, MR_SICK), "sickness");
    ...
    if (oc.oc_class == WEAPON_CLASS || weptool
       ...
       /* const char* sdambon = "";
        ...
        if (dummy.oprops_known) {
            ...
            if (obj->oprops & ITEM_DECAY) OBJPUTSTR("\tdrains life");
            if (obj->oprops & ITEM_SLEEP) OBJPUTSTR("\tsleep attack");
        }
    ...
    if (*dbase_str) {
        ...
        for (pass = !strcmp(alt, dbase_str) ? 0 : 1; pass >= 0; --pass) {
            ...
            if (user_typed_name || without_asking || yes_to_moreinfo) {
                if (!found_in_file && ((!pm && (obj == NULL && otyp == STRANGE_OBJECT)) || !flags.lookup_data)) {
...
static struct {
    ...
    { dispfile_license, "The HackEM license." },
